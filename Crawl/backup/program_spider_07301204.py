...
class ProgramSpider(scrapy.Spider):
            ...
            if depth < 1 or is_root:
                ...
                for link_url, anchor_text, matched_keyword in links:
                    if not filter_url(link_url, anchor_text=anchor_text):
                        self.logger.info(f"[{project_id}] 爬取子链接: {link_url} (锚文本: '{anchor_text}', 匹配关键词: '{matched_keyword}')")
                ...
                if new_requests:
                    ...
                    self.logger.info(f"[{project_id}] 添加 {len(new_requests)} 个新请求，当前剩余: {self.request_counters[project_id] - 1}")
    ...
    def handle_error(self, failure):
        ...
        if project_id:
            ...
            if self.request_counters[project_id] <= 0:
                # 标记项目需要完成，但不在这里直接处理
                # 因为handle_error不是生成器函数，无法yield
                self.logger.info(f"[{project_id}] 项目在错误处理中完成，将在下一个解析周期处理")
                
                # 将项目标记为待完成状态
                if project_id not in getattr(self, '_projects_to_complete', set()):
                    if not hasattr(self, '_projects_to_complete'):
                        self._projects_to_complete = set()
                    self._projects_to_complete.add(project_id)
    ...
    def complete_project(self, project_id):
        ...
        # 在完成当前项目后，检查是否需要开始下一个项目
        if self.project_queue:
            self.logger.info(f"[{project_id}] 开始启动下一个项目，剩余 {len(self.project_queue)} 个项目")
            next_project_requests = self.start_next_project()
            for request in next_project_requests:
                yield request
        else:
            self.logger.info("\n" + "="*50)
            self.logger.info("所有项目已完成")
